...
from agent import SARSALearnerAgent


def run_episode(env: Env, agent, training: bool, gamma) -> float:
    """
    Warning : This is the interaction loop for Q-Learning not SARSA.
    It will not work with the SARSALearnerAgent and was given to you to help you implement run_episode_sarsa.
    ...
    while not done:
        ...
        if training:
            # WARNING: running this will not work.
...
def run_episode_sarsa(env: Env, agent: SARSALearnerAgent, training: bool, gamma) -> float:
    """
    This is for the SARSA algorithm: you should implement this function.
    Interact with the environment for one episode using actions derived from the q_table and the action_selector.
...
def train(env: Env, gamma: float, num_episodes: int, evaluate_every: int, num_evaluation_episodes: int,
    ...
    agent = SARSALearnerAgent(env.observation_space.n, env.action_space.n, alpha, gamma, epsilon_max,
